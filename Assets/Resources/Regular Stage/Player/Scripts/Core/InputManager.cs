...
public class InputManager : MonoBehaviour
{
    ...
    [SerializeField]
    private ButtonInputData flowStateButton;
    ...
    private void Awake()
    {
        ...
        this.inputMaster.Player.FlowState.started += ctx => this.flowStateButton.OnInputPress(ctx.phase, this.IgnorePlayerInput());
        this.inputMaster.Player.FlowState.performed += ctx => this.flowStateButton.OnInputPress(ctx.phase, this.IgnorePlayerInput());
        this.inputMaster.Player.FlowState.canceled += ctx => this.flowStateButton.OnInputPress(ctx.phase, this.IgnorePlayerInput());
    ...
    /// <summary>
    /// When the pause button is pressed and what it means based on the state of the current scene
    /// </summary>
    private void OnPauseButtonPressed(InputAction.CallbackContext ctx)
    {
        ...
        if (this.jumpButton.GetInputActionPhase() == InputActionPhase.Performed)
        {
            return;
        }

        GMPauseMenuManager.Instance().OnPauseButtonPress(ctx.phase);
    }
    ...
    /// <summary>
    /// Set the directional input of the player object
    /// </summary>
    ...
    /// <summary>
    /// Get the flow state button
    /// </summary>
    public ButtonInputData GetFlowStateButton() => this.flowStateButton;
    ...
    /// <summary>
    /// Stop vibrating the active gamepad
    /// </summary>
