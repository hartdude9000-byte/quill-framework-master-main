...
public class AfterImageController : MonoBehaviour
{
    ...
    public void UpdateAfterImages()
    {
        ...
        VelocityCore velocityCore = this.player.GetComponent<VelocityCore>();
        if (velocityCore != null)
        {
            flowStateActive = velocityCore.IsFlowStateActive();
        }

        // If the player is super or has power sneakers or is in Flow State, display the after image
        bool displayAfterImages = this.player.GetHedgePowerUpManager().GetPowerSneakersPowerUp().PowerUpIsActive() 
            || this.player.GetHedgePowerUpManager().GetSuperPowerUp() == SuperPowerUp.SuperForm
            || flowStateActive;
    ...
    {
        Transform afterImagePool = new GameObject().transform;
        ...
        {
            AfterImageData afterImage = Instantiate(this.afterImagePrefab).AddComponent<AfterImageData>();
