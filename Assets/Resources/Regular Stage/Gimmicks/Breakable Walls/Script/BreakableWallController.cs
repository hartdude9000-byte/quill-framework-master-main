...
public class BreakableWallController : SolidContactGimmick
{
    ...
    public override bool HedgeIsCollisionValid(Player player, Bounds solidBoxColliderBounds)
    {
        ...
        switch (this.breakableDirection)
        {
            case BreakableDirection.Horizontal:
                ...
                // Speed = Power: Player moving fast enough breaks walls
                if (playerHasKillSpeed && this.TargetBoundsAreWithinVerticalBounds(this.boxCollider2D.bounds, solidBoxColliderBounds))
                {
                    this.breakDirection = this.TargetIsToTheRight(solidBoxColliderBounds) && player.velocity.x < 0 ? 1 : -1;
                    triggerAction = true;
                    break;
                }
            ...
            case BreakableDirection.Vertical:
                if (this.mustBeAttackedThrough && player.GetAttackingState() == false && !playerHasKillSpeed)
                {
                    break;
                }
                    ...
                    && Mathf.Abs(player.velocity.y) > 0)
                {
                    ...
                    {
                        if (player.GetAttackingState() || playerHasKillSpeed)
                        {
                            triggerAction = true;
    ...
    private void BreakWall()
    {
        ...
        shardSet.transform.position = this.transform.position;
        shardSet.transform.localScale = new Vector3(this.breakDirection, 1, 1);
        BreakableShardSetController breakableShardSetController = shardSet.GetComponent<BreakableShardSetController>();
        breakableShardSetController.OnObjectSpawn();
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.color = this.debugColour;
        Gizmos.DrawCube(this.boxCollider2D.bounds.center, this.boxCollider2D.bounds.size);
    }
}